    ...
    if (hash) {
      ...
      if (access_token) {
        ...
        .then(data => {
          ...
          if (user) {
            document.cookie = `ran_token=${access_token}; path=/; max-age=2592000; SameSite=Lax`
            document.cookie = `ran_user=${encodeURIComponent(JSON.stringify({ id: user.id, email: user.email, name: user.full_name || '' }))}; path=/; max-age=2592000; SameSite=Lax`
          }
